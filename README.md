...
- **CDC 153 & Galaxy Outbreak Sheets**: `rel_gap = 0.2980 - 0.5299` ($\ge 0.2200$), `min_cluster_size` = $52-98$ ($\ge 10\% \cdot N$) $\rightarrow$ **$k = 2$ ($100\%$ Outbreak Accuracy across 6/6 sheets)**.
- **Single-Outbreak Null Cohorts (Vendor_A, Vendor_B & Subsamples)**: `min_cluster_size` = $1-4$ ($< 10\% \cdot N$) $\rightarrow$ **$k = 1$ (Correct Single Group across 10/10 null cohorts)**.
