...
\begin{tikzpicture}[node distance=1.4cm, auto, >=latex', every node/.style={transform shape}]
    ...
    \node [block, right=1.0cm of m1] (m2) {\textbf{Module 2: PyEuk Distance Engine}\\Numba JIT KING-wIBS Kernel $\rightarrow$ SoftImpute SVD Matrix Completion $\rightarrow$ PSD Matrix};
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Module 2 (Numba JIT Parallel KING-wIBS Distance Engine):} Replaces heuristic distance with a weighted Identity-By-State (KING-wIBS) C-kernel and completes missing multi-locus data using SoftImpute SVD nuclear norm minimization.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Massive Parallel Acceleration:} Vectorized Numba JIT kernels process 1,078 samples in 14.92 seconds ($99.2\times$ faster than legacy R).
\end{itemize}
\end{conceptbox}
...
\subsection{Module 2: Numba JIT KING-wIBS Kernel \& SoftImpute SVD Completion}

\subsubsection{1. Operational Mechanics (WHAT the Algorithm Does)}
Module 2 calculates pairwise genetic dissimilarity using a parallel Numba JIT KING-wIBS kernel:
\begin{equation}
...
\begin{tikzpicture}[node distance=1.4cm, auto, >=latex', every node/.style={transform shape}]
    ...
    \node [block, right=0.8cm of matrix] (numba) {Numba JIT C-Kernel\\Parallel KING-wIBS ($<50\text{ ms}$)};
    ...
    \path [line] (soft) -- (out);
\end{tikzpicture}
\caption{Parallel Numba JIT wIBS and SoftImpute SVD Matrix Completion Pipeline.}
\label{fig:wibs_flow}
\end{figure}
